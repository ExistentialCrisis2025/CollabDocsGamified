...
const ProfilePage = () => {
  ...
  const claimQuest = async (questId: number) => {
    try {
      ...
      toast.error(apiError.response?.data?.error || "Failed to claim quest");
    } finally {
      setClaiming(null);
    }
  };
  ...
  const totalXp = dashboardData?.user?.total_xp || 0;
  const nextLevelXpRaw = dashboardData?.user?.next_level_xp;
  const nextLevelXp =
    typeof nextLevelXpRaw === "number" && nextLevelXpRaw > 0
      ? nextLevelXpRaw
      : null;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 transition-colors dark:bg-slate-900 dark:text-slate-100">
      ...
      >
        <motion.div variants={itemVariants} className="mb-6">
          <Link
            to="/dashboard"
            className="flex items-center gap-2 text-indigo-500 font-medium hover:underline w-max"
          >
        ...
        <motion.div
          ...
          className="mb-12 flex flex-col items-center sm:flex-row gap-8 bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700"
        >
          <div className="relative group cursor-pointer">
            <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-indigo-500 bg-slate-200 dark:bg-slate-700 flex items-center justify-center">
              <img
                src={`https://api.dicebear.com/7.x/bottts/svg?seed=${dashboardData?.user?.username}&backgroundColor=6366f1`}
                alt="Profile"
            ...
            <div className="w-full">
              ...
              <div className="h-4 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                <div
                  ...
                  style={{
                    width: `${nextLevelXp ? Math.min((totalXp / nextLevelXp) * 100, 100) : 100}%`,
                  }}
                ></div>
              </div>
            </div>
        ...
        <motion.div
          variants={itemVariants}
          className="mb-8 rounded-3xl border border-indigo-200/50 dark:border-indigo-500/30 bg-white dark:bg-slate-800 p-8 shadow-xl"
        >
            ...
            {badges.length > 0 ? (
              badges.map((badge) => {
                const rarityColors: Record<string, string> = {
                  common:
                    "border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800 text-slate-500",
                  ...
                  legendary:
                    "border-orange-300 bg-orange-50 dark:border-orange-500/50 dark:bg-orange-900/50 text-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.2)]",
                };
                ...
                const cardStyle = rarityColors[rarity] || rarityColors.common;
                const iconGradient =
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-16 h-16 rounded-full bg-gradient-to-br ${iconGradient} flex items-center justify-center mb-3 shadow-lg`}
                    >
                      <Trophy className="w-8 h-8 text-white drop-shadow-md" />
                    </div>
                    <h3 className="font-bold text-slate-900 dark:text-white capitalize text-sm mb-1">
                      {badge.name}
                    </h3>
